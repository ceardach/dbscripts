...
#**************************************************************************
#
# Modifications made by Kathleen Murtagh, kathleen@ceardach.com
# For use in the Drupal module, dbscripts
#
#**************************************************************************
...
function parse_csv(string,csv,sep,quote,escape,newline,trim, fields,pos,strtrim) {
    ...
    while (length(string) > 0) {
        ...
        quoted = 0; # Used to put quotes back on strings after parsing
        ...
        if (trim && pos != length(string) && substr(string, pos + strtrim, 1) == " ") {
            trim = strtrim
            # Count the number of spaces found.
        ...
        # Gather the field.
        if (quoted == 1) {
          # Put the quotes back so they remain as strings
